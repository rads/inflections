...
(defn- parse-number [s parse-fn]
  (when-let [matches (re-matches #"\s*([-+]?[0-9]*\.?[0-9]+([eE][-+]?[0-9]+)?)(M|B)?.*" (str s))]
    (let [number (parse-fn (nth matches 1))
          unit (nth matches 3)]
      (when-not #?(:clj (Double/isNaN number) :cljs (js/isNaN number))
        (apply-unit number unit)))))

(defn- parse-integer
  "Parse `s` as a integer number."
  [s]
  (parse-number s #?(:clj #(Integer/parseInt %1) :cljs #(js/parseInt %1))))
